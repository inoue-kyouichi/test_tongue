 ...
 */
void SmoothedFEM::SFEM::calcStressTensor_linearElasticMaterial_element_spatialForm(const int ic,ARRAY2D<double> &U_tmp,const bool option)
{
  ...
  switch(element[ic].meshType){
    case VTK_TETRA:
      ...
      linearElasticMaterial_inGaussIntegral(dNdr,x_current,x_ref,numOfNodeInElm,gTet.weight[0]*1e0/6e0,ic,option);
      break;
    case VTK_HEXAHEDRON:
      for(int i1=0;i1<2;i1++){
        for(int i2=0;i2<2;i2++){
          for(int i3=0;i3<2;i3++){
            ...
            linearElasticMaterial_inGaussIntegral(dNdr,x_current,x_ref,numOfNodeInElm,g.weight[i1]*g.weight[i2]*g.weight[i3],ic,option);
          }
        }
      }
      break;
    case VTK_QUADRATIC_TETRA:
      for(int i1=0;i1<4;i1++){
        ...
        linearElasticMaterial_inGaussIntegral(dNdr,x_current,x_ref,numOfNodeInElm,gTet2.weight[i1]*1e0/6e0,ic,option);
      }
      break;
    ...
    case VTK_WEDGE:
      for(int i1=0;i1<2;i1++){
        ...
        linearElasticMaterial_inGaussIntegral(dNdr,x_current,x_ref,numOfNodeInElm,5e-1*gTri.weight[0]*g.weight[i1],ic,option);
      }
      break;
    default:
 ...
 * @param [in] option           true or faluse: calculate tangential stiffness matrix or not.
 */
double SmoothedFEM::SFEM::linearElasticMaterial_inGaussIntegral(ARRAY2D<double> &dNdr,ARRAY2D<double> &x_current,ARRAY2D<double> &x_ref,
